...
export async function GET(request: NextRequest) {
  ...
  const resultsCount = await recipeIngredient
    .find({ desc: { $regex: new RegExp(regex) } })
    .sort({ createdAt: "desc" })
    .count()
    .exec();
  ...
  return NextResponse.json(
    { recipeIngredients: recipeIngredients, resultsCount: resultsCount },
    { status: 200 }
  );
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (recipeCheck && ingredientCheck) {
    try {
      ...
      return NextResponse.json(
        { recipeIngredient: { _id: result?._id, ...data } },
        { status: 200 }
      );
    } catch (error: any) {
      console.log(error.message);
