...
export default function Ingredients(){ 
    ...
    return(
        <>
            ...
            <section id="admin-ingredients-section" className='max-w-7xl bg-slate-500/30 mx-auto w-full p-4 md:p-8 rounded-xl shadow-md shadow-black/40 flex-col'>
                <div className="flex flex-col gap-4 overflow-visible ">
                    <div className="flex justify-between place-items-center border-b-2 pb-6 border-violet-50/40 text-white">
                        <h1 className="text-2xl font-semibold"> <Link href={"/admin/"} className="opacity-70 hover:opacity-100">Admin Panel</Link> / Ingredients</h1>
                ...
                <FullScreenPopup>
                    ...
                    <div className="flex justify-end gap-2 text-white mt-4">
                        <Button className="bg-red-600" onClick={() => DeleteIngredient(selectedId)}>Delete</Button>
    ...
    return(
        <li className={`${dropdownOpen ? "!bg-gray-600" : ""} flex relative overflow-y-visible w-full min-w-fit gap-4 place-items-center p-3 font-normal hover:!bg-gray-600 border-violet-50/40 text-white`}>
            <span className="w-full min-w-54 cursor-pointer">
                <Link href={"/admin/ingredients/view/" + ingredient?._id} className="flex place-items-center gap-1" target="_blank">
                {ingredient.name}
                </Link>
                </span>
            <span className="w-28 shrink-0 max-xs:hidden overflow-hidden overflow-ellipsis">{ingredient.vegan && <FaCheckCircle  className="w-5 h-5 fill-green-500"/>}</span>
            <span className="w-28 shrink-0 max-xs:hidden overflow-hidden overflow-ellipsis">{ingredient.vegetarian && <FaCheckCircle  className="w-5 h-5 fill-green-500"/>}</span>
            <span className="w-28 shrink-0 max-xs:hidden">{ingredient?.createdAt ? ParseDate(ingredient?.createdAt) : "Unknown"}</span>
            ...
            <OptionsDropdown dropdownOpen={dropdownOpen} setDropdownOpen={setDropdownOpen} refValue={ref} id={ingredient._id ?? ""}/>
        </li>
    )
}
    ...
    return(
        <ul className={`${dropdownOpen ? "block" : "hidden"} bg-[#13171c] w-44 h-fit absolute right-0 -bottom-44 z-20 rounded-lg shadow-md border-2 border-white/30`}
            ...
            <li className="p-4 hover:bg-slate-400/10 cursor-pointer select-none">
                <Link href={"/admin/ingredients/view/" + id} className="flex place-items-center gap-1" target="_blank">
                    <FaEye/> View
                </Link>
            </li>
            <li className="p-4 hover:bg-slate-400/10 cursor-pointer select-none">
                <Link href={"/admin/ingredients/edit/" + id} className="flex place-items-center gap-1" target="_blank">
                    <FaEdit/> Edit
                </Link>
            </li>
