            ...
            <main id="admin-recipe-view-section" className='max-w-4xl bg-slate-500/30 mx-auto w-full max-sm:p-4 p-8 rounded-xl shadow-md shadow-black/40 flex flex-col gap-6' onSubmit={(e) => submitAction(e)}>
                <section className="flex flex-col gap-8">
                    <div className="flex justify-between gap-8">
                        {loading ?
                            <h1 className="bg-coal-400 text-transparent rounded-lg w-full animate-pulse">No data</h1> :
                            <h1 className="text-xl md:text-2xl font-bold flex max-xs:flex-col-reverse place-items-start">
                                <div className="flex">
                                {recipe.vegetarian && <span className="relative w-7 h-7 pt-1 shrink-0"><FaFish className="absolute p-1"/> <FaBan className="absolute fill-red-500"/> </span>}
                                {recipe.vegan && <span className="relative w-7 h-7 pt-1 shrink-0"><FaEgg className="absolute p-1"/> <FaBan className="absolute fill-red-500"/> </span>}
                                </div>
                                {recipe?.title}
                            </h1>
                        }
                    ...
                    {loading ?
                        <h1 className="bg-coal-400 h-32 text-transparent rounded-lg w-full animate-pulse">No data</h1> :
                    ...
                    <div className="flex flex-col gap-2">
                        <p className="flex whitespace-nowrap justify-between place-items-center gap-2 w-48"> 
                            ...
                            {loading ?
                            <span className="bg-coal-400 text-transparent rounded-lg w-14 animate-pulse">Loading...</span> :
                ...
                <section>
                    <div className="w-full relative">
                        ...
                        {loading ?
                            <span className="bg-coal-400 text-transparent h-16 rounded-lg w-full animate-pulse">Loading...</span> :
                ...
                <div className="w-full relative">
                        ...
                        {loading ?
                            <span className="bg-coal-400 text-transparent h-64 rounded-lg w-full animate-pulse">Loading...</span> :
                            parse((!recipe.recipeDetails.desc || recipe.recipeDetails.desc == "<p></p>") ? "<p className='opacity-80'>Nic tu nie ma.</p>" : recipe.recipeDetails.desc)
                        }
                        </div>
                    </div>
                </section>
