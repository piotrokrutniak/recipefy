...
export function AddIngredient({
  ...
  setIngredients: Dispatch<SetStateAction<RecipeType>>;
  id: string;
}) {
  ...
  return (
    <>
      <div className="flex justify-end">
        <Button className="mt-8 w-fit" onClick={() => setPopupOpen(true)}>
          {" "}
      ...
      {popupOpen && (
        <FullScreenPopup className="relative">
          <BsX
            className="absolute top-2 right-2 h-10 w-10 cursor-pointer active:opacity-70 hover:fill-red-500"
          ...
          <h1 className="font-bold text-xl mb-4 flex gap-2 place-items-center">
            ...
            <Link
              ...
              target="_blank"
              className="w-fit flex gap-2 place-items-center rounded-lg font-normal text-base !p-2 bg-transparent border-2 border-solid border-white/40 hover:border-white active:opacity-80 transition-all"
            >
              <FaPlusCircle /> Create
            </Link>
          </h1>
          {selectedIngredient ? (
            <div className="flex flex-col">
              ...
              <div className="p-2 bg-coal-400 rounded-lg flex justify-between place-items-center">
                ...
                <BsX
                  className="h-8 w-8 cursor-pointer active:opacity-70 hover:fill-red-500"
                  onClick={() => setSelectedIngredient(undefined)}
                />
              </div>
            </div>
          ) : (
            <div className="flex flex-col relative" ref={resultsRef}>
              ...
              />
              <div
                className={`${resultsOpen ? "h-44 border-sky-100/50" : "h-0 border-none"} border-2 transition-all w-full bg-[#3d4756] rounded-lg absolute top-24 overflow-y-scroll`}
              >
                {results.length ? (
                  results?.map((x, key) => (
                    <IngredientListing
          ...
          <Button className="mt-6 w-fit ml-auto mr-0" onClick={() => AddRecipeIngredient()}>
            {" "}
            <FaPlusCircle /> Add{" "}
          </Button>
        </FullScreenPopup>
