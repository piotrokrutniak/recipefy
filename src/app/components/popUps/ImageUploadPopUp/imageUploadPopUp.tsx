...
export default function ImageUploadPopUp({
  ...
  return (
    <FormPopup
      ...
      uploadFunction={() => console.log("click")}
      setPopUpOpen={setUploadOpen}
      popupOpen={uploadOpen}
      triggerDiscard={ImageUploadDiscard}
      triggerSave={SaveThumbnail}
    >
      ...
      <div className="flex aspect-square w-full bg-slate-400/30 relative rounded-lg overflow-hidden place-items-center justify-center">
        {fileToUpload ? (
          ...
          <BsImageFill className="w-24 h-24 opacity-50" />
        )}
        {fileToUpload ? (
          <BsX
            className="absolute top-0 right-0 w-10 h-10 cursor-pointer opacity-60 hover:opacity-100 transition-all active:opacity-30 duration-100"
